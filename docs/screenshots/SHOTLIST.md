...
The install guide (`docs/GwinnIQ-Install-Guide.pptx`) and the README leave labeled
...
These three appear as placeholder frames in `GwinnIQ-Install-Guide.pptx`. Replace each frame
...
| **3** | `03-copilot-studio-agent.png` | Copilot Studio → *your GwinnIQ agent* | The agent open with the instructions pasted and the custom connector's actions listed under **Tools / Actions**. The GwinnIQ avatar as the icon is a nice touch to include. |
...
| `08-agent-share.png` | Copilot Studio → *your GwinnIQ agent* → **Share** | The share dialog granting a reviewer access to the published agent. |
| `09-agent-preview.png` | Copilot Studio → *your GwinnIQ agent* → **Test / Preview** | A short conversation: “list my cases”, then a create request where the agent **restates and waits** for confirmation. The single best shot for showing the agent in action. |
...
PowerPoint: open `GwinnIQ-Install-Guide.pptx`, delete the dashed placeholder on the Phase 1–3

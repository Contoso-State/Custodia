...
>
> A 12-slide walkthrough written for state and county teams: what Custodia does, why it is safe
...
bound, and pick a broader scope than they meant to. Custodia gives them a conversational path
...
```mermaid
flowchart LR
    R[Reviewer in Teams /<br/>M365 Copilot] --> A[Copilot Studio agent<br/>Custodia]
...
- **Delegated permissions only.** The agent can never exceed the signed-in reviewer's own
  ...
  see it through Custodia either.
...
- **eDiscovery Administrator** — sees *every* case in the tenant. Reserve this for the case-
  management lead. Note that Custodia inherits it: an admin using the agent can list and read
...
| --- | --- | --- |
| `custodia-eval-core-20.csv` | 20 | The FOIA workflow: create, search, estimate, collect, export |
| `custodia-eval-guardrails-10.csv` | 10 | Refusals — purge, delete, holds, legal determinations, prompt injection |
| `custodia-eval-verify-5.csv` | 5 | Fast, no-async smoke checks; use to isolate pipeline problems |
...
python src/custodia_ediscovery.py \
...
Only after all three pass should Custodia touch a live request.
